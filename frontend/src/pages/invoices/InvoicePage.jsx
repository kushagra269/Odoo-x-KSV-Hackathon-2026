  ...
  const markPaidMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({
        ...
        description: "This invoice is now marked as paid in the backend.",
      });
    },
  });
  const emailMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      pushToast({
        ...
        description: response.message || "The invoice email was recorded in the backend.",
      });
    },
  });
      ...
      <SurfaceCard className="page-stack">
        ...
        <div className="invoice-status">
          <div className="invoice-status__meta">
            <span>Status</span>
            <StatusBadge status={invoice?.status}>{invoice?.status?.replaceAll("_", " ")}</StatusBadge>
          </div>
          <Button
            disabled={!invoice || invoice.status === "paid" || markPaidMutation.isPending}
            onClick={() => markPaidMutation.mutate()}
          >
            {invoice?.status === "paid" ? "Already Paid" : markPaidMutation.isPending ? "Updating..." : "Mark as Paid"}
          </Button>
        </div>
      </SurfaceCard>
    </div>
